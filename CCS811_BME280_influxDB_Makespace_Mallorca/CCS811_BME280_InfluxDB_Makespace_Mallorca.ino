...
// Medir eCO2, TVOC, presión, humedad & temperatura ambiental con Wemos D1 mini (lite), CCS811 y BME280
//
// credits to:
...
unsigned long PERIOD = 5000; // periodo de captura en millisegundos, 180000 = 180 s = 3 min
String colsep = ";"; // separador columna de datos CSV mediante serial
bool subirAinflux = false; // poner a false si no quereís subir los datos al servidor influx
...
#define PIN_NOT_WAKE D3 // nWAKE Pin en D3 (GPIO0)
...
//    }
    ...
    configTime(timezone * 3600, dst * 0, ntpServer1, ntpServer2);
    Serial.println("\nWaiting for time");
    while (!time(nullptr)) {
      Serial.print(".");
      delay(1000);
